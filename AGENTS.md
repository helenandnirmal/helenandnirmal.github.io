...
- Always recommend a plan before making edits.
- Do not make code edits until the user has explicitly confirmed they want to proceed with the changes.
- When a plan is needed, present the proposed scope, risk, and next steps, then wait for approval before editing files.
- Keep edits limited to the approved plan; do not expand scope without explicit confirmation.
